use crate::string::queue_item::QueueItem;
...
pub struct StringGameData {
    #[serde(rename = "serialNumber")]
    serial_number: [u8; 9],
}
#[wasm_bindgen]
impl StringGameData {
    pub fn new(serial_number: JsValue) -> Option<StringGameData> {
    ...
    pub fn random() -> Self {
        ...
        seen_state.insert(serial_number.iter().fold(String::new(), |mut acc, x| {
        ...
        while let Some(q_item) = queue.pop_front() {
            ...
            q_item.next().into_iter().for_each(|new_item| {
                if !seen_state.contains(&new_item.now_state.iter().fold(
                    String::new(),
                    |mut acc, x| {
                        acc.push_str(&x.to_string());
                        acc
                    ...
                    seen_state.insert(new_item.now_state.iter().fold(
                        String::new(),
                        |mut acc, x| {
                            acc.push_str(&x.to_string());
                            acc
...
#[cfg(test)]
mod test {
    use super::StringGameData;

    #[test]
    fn test_get_steps() {
        let game_data = StringGameData {
